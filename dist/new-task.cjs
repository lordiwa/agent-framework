...
module.exports = __toCommonJS(new_task_exports);
...
async function runQuestionnaire({
  ...
  let answers = {};
  if (persistTo && (0, import_node_fs3.existsSync)(persistTo)) {
    try {
      const raw = JSON.parse((0, import_node_fs3.readFileSync)(persistTo, "utf8"));
...
function adaptPrompter(legacyPrompter) {
  return async (ctx) => {
    const text = ctx.error ? `(${ctx.error}) ${ctx.prompt} ` : `${ctx.prompt} `;
    return legacyPrompter(text);
  };
}
async function runCli({ argv, prompter, repoRoot, now }) {
